...
const Views = {
  ...
  async _clearOrders() {
    if (!confirm('Clear ALL orders from your history? This cannot be undone.')) return;
    try {
...
const Admin = {
  ...
  async _orders() {
    ...
    return `<div class="admin-head"><h2>Orders</h2><p>${list.length} total · <span style="color:var(--gold);">${pending} pending</span></p></div>
      <p style="font-size:.68rem;color:var(--text-3);margin-bottom:10px;">⚠️ Affiliate commissions are only credited when you mark an order as <strong>Completed</strong>.</p>
      ...
      <div style="overflow-x:auto;"><table class="admin-table">
        ...
        <tbody>${list.map(o => `<tr>
          ...
          <td><div style="display:flex;gap:4px;flex-wrap:wrap;">
            ${o.status !== 'completed' && o.status !== 'cancelled' ? `<button class="aff-copy-btn" style="color:var(--teal);border-color:var(--teal);font-size:.65rem;" onclick="Admin._upOrd('${o.id}','completed')">✓ Done</button>` : ''}
            <button class="aff-copy-btn" style="color:var(--red);border-color:var(--red);font-size:.65rem;" onclick="Admin._delOrder('${o.id}')">🗑</button>
          </div></td>
        </tr>`).join('')}</tbody>
      </table></div>`;
  },
    ...
    return `<div class="admin-head"><h2>Withdrawal Requests</h2><p>${pending} pending</p></div>
      ...
      <div style="overflow-x:auto;"><table class="admin-table">
        ...
        <tbody>${wds.map(w => {
          ...
          <td><span class="wd-status wd-${w.status}">${w.status}</span></td>
          <td>${w.status === 'pending' ? `<div style="display:flex;gap:4px;">
            <button class="aff-copy-btn" style="color:var(--teal);border-color:var(--teal);font-size:.65rem;" onclick="Admin._appWd('${w.id}')">✓ Pay</button>
            <button class="aff-copy-btn" style="color:var(--red);border-color:var(--red);font-size:.65rem;" onclick="Admin._rejWd('${w.id}')">✕ Reject</button>
  ...
  async _delOrder(id) {
    if (!confirm('Delete this order? It will be removed from the admin panel.')) return;
  ...
  async _clearAllOrders() {
    if (!confirm('Clear ALL orders from the admin panel? This cannot be undone.')) return;
